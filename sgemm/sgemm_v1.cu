...
template<int Block_Size_X,int Block_Size_Y,int M,int K,int N>
__global__ void gemmV1(float *mat_A,float *mat_B,float *mat_C)
{
    int col = threadIdx.x +  blockIdx.x*blockDim.x;
    int row = threadIdx.y +blockDim.y*blockIdx.y;
    const float *mat_A_start_ptr=mat_A+blockIdx.y*blockDim.y*K;
    const float *mat_B_start_ptr = mat_B + blockDim.x+blockIdx.x;
    __shared__ float mat_A_shared[Block_Size_Y][K];
    __shared__ float mat_B_shared[K][Block_Size_X];
    for (int i = 0; i < K; i+=blockDim.x)
    {
        mat_A_shared[threadIdx.y][threadIdx.x+i] = mat_A_start_ptr[threadIdx.y*K+threadIdx.x+i];
    }
    for (int i = 0; i < K; i+=blockDim.y)
    {
        mat_B_shared[threadIdx.y+i][threadIdx.x] = mat_B_start_ptr[threadIdx.x + (threadIdx.y+i)*N];
    }
    // 卡住线程
    __syncthreads();
    float temp = 0.0f;
...
int main()
{
    ...
    gemmV1<block_size_x,block_size_y,m,k,n><<<Grid,Block>>>(mat_A_gpu,mat_B_gpu,mat_C_gpu);
